    ...
    search: function () {
      ...
      axios.post('/api/search', data)
        .then((response) => {
          ...
          console.log(this.slists[j].category)
          while (i < response.length) { // res
            while (j < this.slists.length) {
              if (response[i].subject == this.slists[j].category) {
                let item = {
                  _id: response[i]._id,
                  title: response[i].title,
                  professor: response[i].professor,
                  method: response[i].method,
                  class: response[i].class
                }
                console.log(response[i].subject)
                console.log(this.slists[j].category)
                this.slists[j].items.append(item)
                break
              }
                j++
            }
